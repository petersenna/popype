...
class Pipeline:
    """An example pipeline:
          ...
          linus.git: step1.cocci|finalize1.py|step2.cocci|post.py > peter.git

          linus.git: defines the git repository and checkout points to use
          *.cocci apply the semantic patch on each checkout of linus.git
          *.py apply the script to stdout of previous semantic patch
          > peter.git saves the output of _all_ pipeline stages there
